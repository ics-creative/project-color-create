...
class ColorRGB {
	...
	private _g:number;
	private _b:number;
	private static zero:string[] = ["なし", "00000", "0000", "000", "00", "0", ""];
	...
	public setRGBString(value:string):void {
		var arr:string[] = value.split("(")[1].split(")")[0].split(",");
	...
	private setData(uintStyle:number) {
		...
		var code:string = uintStyle.toString(16).toUpperCase();
	...
	public static RGBtoCMYK(uint:number):number[] {
		var col:ColorRGB = new ColorRGB(uint);
		var c:number = 1 - col.getR() / 0xFF;
		var m:number = 1 - col.getG() / 0xFF;
		var y:number = 1 - col.getB() / 0xFF;
		var k:number = c > m ? (m > y ? y : m) : (c > y ? y : c);
		var k1:number = 1 - k;
